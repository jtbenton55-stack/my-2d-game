...
## Godot MCP Pro startup / port checks

1. Open the Godot project.
2. Enable **Godot MCP Pro** in **Project -> Project Settings -> Plugins**.
3. Restart Godot.
4. Check ports `6505-6509`:
   `foreach ($p in 6505..6509) { Test-NetConnection 127.0.0.1 -Port $p | Select-Object ComputerName, RemotePort, TcpTestSucceeded }`
5. `6005` is the Godot LSP endpoint.
6. `6006` is the Godot DAP endpoint.
7. `6505` (or any port in `6505-6509`) is the Godot MCP Pro bridge endpoint.
8. If `6505` stays closed, inspect Godot Output/Debugger logs for MCP, WebSocket, or plugin startup errors.
